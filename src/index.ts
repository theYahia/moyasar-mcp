...
server.tool("create_payment", "Create a payment via Moyasar.", createPaymentSchema.shape,
  async (params) => ({ content: [{ type: "text", text: await handleCreatePayment(params) }] }));

server.tool("get_payment", "Get payment details by ID.", getPaymentSchema.shape,
  async (params) => ({ content: [{ type: "text", text: await handleGetPayment(params) }] }));

server.tool("list_payments", "List payments with filters.", listPaymentsSchema.shape,
  async (params) => ({ content: [{ type: "text", text: await handleListPayments(params) }] }));

server.tool("refund", "Refund a captured payment.", refundPaymentSchema.shape,
...
server.tool("list_invoices", "List invoices.", listInvoicesSchema.shape,
  async (params) => ({ content: [{ type: "text", text: await handleListInvoices(params) }] }));

server.tool("void", "Void an authorized payment.", voidPaymentSchema.shape,
  async (params) => ({ content: [{ type: "text", text: await handleVoidPayment(params) }] }));
